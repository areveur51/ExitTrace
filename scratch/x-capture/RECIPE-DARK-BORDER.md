...
3. **RT → original fallback** — `ensureFullTimestampArticle` navigates to the embedded original `/status/ID` when the RT/timeline card lacks full datetime, then re-expands + re-proves.
