...
- A final reply is one plain-text confirmation plus a host screenshot of the public KEEP page, and only when the dig status is `kept`. The text has no URL. Fail-closed, rejected, ambiguous, and dig failures stay silent. A later mention of that subject, or another KEEP that shares the slug, does not get a second final.
