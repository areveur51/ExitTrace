...
| Dump runner | Self-hosted labels `self-hosted`, `exittrace-lab` (the registered lab runner) |
| Dump script | host helper `et-lab-dump.sh` — `docker exec grokbuild-postgres pg_dump -Fc` of database `exittrace` on host **:5433** |
