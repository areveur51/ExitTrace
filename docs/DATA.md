...
If the same person already exists in that category (id/slug, or the same subject with `event_date` within three days), only new cites are attached. Name, date, category, and existing cites stay as they are. Dedup is per kind: the same person may appear on `/arrests` and one indictment list when the events differ (arrest versus indictment). Never two indictment rows of the same classification. The source post stays on Unsorted (it is not deleted). The command is idempotent on Postgres and on the file store. It does not write `data/seed.json`.
