...
`/central-casting` supersedes the KIND_COMMS clip list. It lists one card per existing person KEEP. Membership is `people.central_casting`, a JSON array of cite URLs, with no sense split. Health counts those unique persons, not harvest rows. Harvest rows in `central_casting_comms` carry `person_id` and render inside one Central Casting section on the person detail (the same PersonEventSection as other event kinds): each stored quote sits immediately before its own cite link, with optional supportive media or an X link. An empty snippet or the placeholder `0` is omitted. Bootstrap backfills `text` only from that row's `quote` / `body` / snapshot fields, or from `source_posts.text` when the URL is the same cite. It does not insert a row and does not invent a quote. Empty membership with no cites stays hidden. The section is not a second red-folder page. Fresh envs create the table from `scripts/bootstrap-db.sql`. This repo does not seed rows.
