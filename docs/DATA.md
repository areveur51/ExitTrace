...
People: named subject, calendar `event_date` (never copied from `posted_at`), catalog category (including `arrests`), and ≥2 verified official news or official government / news-org social URLs. Unofficial or commentary social is extra only, not a cite. `/add` may include an optional Wikimedia or official `.gov` portrait URL; ineligible URLs are rejected at queue. At process time the same create/annotate helper attaches a local file under `/media/people/` when that eligible still exists. Photos are not invented. Existing gold photos are not overwritten. A missing still stays blank. If the request `hint_url` matches one parked Unsorted source post, the same fail-closed insert helper is reused. The Unsorted classify walk (`import-posts` / `promote`) stays a separate path. Gold rows stay annotate-only (name, date, category, existing cites, and existing photos are not overwritten). The committed seed stays 72 people; a live store may already have 73.
