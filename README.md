...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open one timeline-style column masonry: media tiles (portrait + X screenshot + other post media) interleaved with text/meta cards (title, handle, account, posted, body, source_url). CSS columns pack densely (<720 1-col; ≥720 2-col; ≥1100 3-col) so tiles fill gaps. Media tiles open a shared HD lightbox; text/meta tiles do not. X URL only under Source; no duplicate Sources pane. People (including corona), operations, and dog-comm detail share one `detailShell` + `detailMediaStrip` + `detailMetaBlock` masonry over glass-transparent frost (light blur) so the city skyline stays readable. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
