...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open Metadata TUI lines (handle, account, posted, body, source_url) stacked under the still and optional screenshot (shared lightbox; no duplicate Sources pane). People (including corona), operations, and dog-comm detail share one `detailShell` + media strip + meta block stack over the deepened Glass void/frost/city-wireframe/gold schematic. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
