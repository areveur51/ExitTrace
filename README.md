...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open one interleaved masonry of media tiles (portrait + X screenshot + other post media) plus text/meta tiles (handle, account, posted, body, source_url). Columns pack densely (<720 1-col; ≥720 2-col; ≥1100 3-col; media min-height 280px on 1-col). Media tiles open a shared HD lightbox; text/meta tiles do not. X URL only under Source; no duplicate Sources pane. People (including corona), operations, and dog-comm detail share one `detailShell` + `detailMediaStrip` + `detailMetaBlock` masonry over glassier Glass frost so the city wireframe stays clearly visible. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
