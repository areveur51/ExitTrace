...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open Metadata TUI lines (handle, account, posted, body, source_url) stacked under a larger masonry grid of portrait + X screenshot + other post media (<720 1-col min-height 280px; 720–1099 2-col minmax(340px,1fr); ≥1100 2-col minmax(420px,1fr); 3-col only ≥1400 when ≥3 tiles; primary X screenshot may span 2 cols; shared HD lightbox; X URL only under Source; no duplicate Sources pane). People (including corona), operations, and dog-comm detail share one `detailShell` + masonry media + meta block stack over translucent Glass frost so the city wireframe stays visible. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
