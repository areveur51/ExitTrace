...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open Metadata TUI lines (handle, account, posted, body, source) with the still and optional screenshot in the shared lightbox. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
