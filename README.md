...
Dog comms store the post text, poster handle, date, and a local still when one is freely licensed. The source URL is a citation only. Tap a dog-comm row to open Metadata TUI lines (handle, account, posted, body, source_url) stacked under a masonry of portrait + X screenshot + other post media (shared HD lightbox; X URL only under Source; no duplicate Sources pane). People (including corona), operations, and dog-comm detail share one `detailShell` + masonry media + meta block stack over translucent Glass frost so the city wireframe stays visible. The pages do not load `widgets.js` and do not fetch X, Wikimedia, or news sites at view time.
