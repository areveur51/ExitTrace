...
test("dog-comms page paginates stored rows and opens TUI meta detail", async () => {
  ...
  assert.match(detail.body, /class="handle"/);
  assert.match(detail.body, /class="acct"/);
  assert.match(detail.body, /class="post-text"/);
  assert.match(detail.body, /Source ·/);
  assert.match(detail.body, /detail-tile--cite/);
  assert.match(detail.body, /detail-tile--source/);
  assert.doesNotMatch(detail.body, /detail-tile--handle|detail-tile--account|detail-tile--posted|detail-tile--body/);
  assert.doesNotMatch(detail.body, /Handle ·|Account ·|Posted ·|Body ·/);
