...
test("CSS tokens cover Glass only and keep schematic HUD chrome", () => {
  ...
  assert.match(css, /url\("\/media\/themes\/glass-callsign\.webp"\)/);
  assert.match(
    css,
    /url\("\/media\/themes\/glass-callsign\.webp"\),[\s\S]*?url\("\/media\/themes\/glass-bg\.webp"\)/,
  );
  ...
  assert.ok(
    fs.existsSync(path.join(ROOT, "app", "public", "media", "themes", "glass-callsign.webp")),
    "glass call-sign overlay is vendored locally",
  );
