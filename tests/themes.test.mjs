...
test("layout defaults to Glass with real catalog routes and no theme picker", () => {
  ...
  assert.match(footer, /href="\/central-casting-comms"/);
  assert.match(footer, /\]<\/span> Central Casting</);
...
test("CSS tokens cover Glass only and keep schematic HUD chrome", () => {
  ...
  assert.match(css, /column-count:\s*1/);
  assert.match(css, /column-count:\s*2/);
  assert.match(css, /column-count:\s*3/);
  assert.match(css, /\.detail-media--masonry \{[^}]*flex:\s*1 1 auto/);
  assert.match(css, /\.meta-pane--stack \.detail-media--masonry \{[^}]*width:\s*100%/);
  assert.doesNotMatch(css, /data-tiles="2"/);
  assert.doesNotMatch(css, /minmax\(340px, 1fr\)/);
  assert.doesNotMatch(css, /minmax\(420px, 1fr\)/);
  assert.doesNotMatch(css, /detail-media--tiles-3/);
  assert.doesNotMatch(css, /grid-column:\s*span 2/);
  assert.match(css, /rgba\(6,\s*8,\s*12,\s*0\.08\)/);
  assert.match(css, /blur\(2px\)/);
  assert.match(css, /blur\(1\.5px\)/);
  assert.doesNotMatch(css, /rgba\(6,\s*8,\s*12,\s*0\.34\)/);
  assert.doesNotMatch(css, /rgba\(6,\s*8,\s*12,\s*0\.16\)/);
  assert.doesNotMatch(css, /blur\(10px\)/);
  assert.doesNotMatch(css, /blur\(4px\)/);
  ...
  assert.match(css, /city-wireframe|city still|glass-bg\.webp/);
  ...
  assert.doesNotMatch(css, /url\("\/media\/themes\/glass-callsign\.png"\)/);
  assert.doesNotMatch(css, /\.home-callsign/);
  ...
  assert.match(
    css,
    /@media \(min-width: 1101px\)[\s\S]*\.tui-main,\s*\.home-stage \{[\s\S]*grid-column:\s*1;[\s\S]*grid-row:\s*1;/,
  );
  assert.match(
    css,
    /@media \(min-width: 1101px\)[\s\S]*\.keymap \{[\s\S]*grid-column:\s*2;[\s\S]*grid-row:\s*1;[\s\S]*gap:\s*0\.1rem;/,
  );
  assert.match(
    css,
    /@media \(min-width: 1101px\)[\s\S]*\.keymap-keys \{[\s\S]*flex:\s*0 0 auto;[\s\S]*gap:\s*0\.1rem;/,
  );
  assert.match(
    css,
    /@media \(min-width: 1101px\)[\s\S]*\.keymap \.fineprint \{[\s\S]*flex:\s*0 0 auto;/,
  );
  assert.match(
    css,
  ...
  assert.ok(fs.existsSync(city), "glass theme still is vendored locally");
  assert.equal(
    fs.statSync(city).size,
    650650,
    "city still is the Admiral-signed webp (baked-in signal)",
  );
  assert.equal(fs.existsSync(path.join(ROOT, "app", "public", "media", "themes", "glass-callsign.png")), false);
